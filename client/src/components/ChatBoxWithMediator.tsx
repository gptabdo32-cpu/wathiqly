...
import { Send, Paperclip, Mic, X, Download, Shield } from "lucide-react";
import { toast } from "sonner";
import RequestMediatorButton from "./RequestMediatorButton";
...
interface Message {
  ...
  createdAt: Date;
  isMediatorMessage?: boolean;
}

interface ChatBoxWithMediatorProps {
...
export default function ChatBoxWithMediator({
  conversationId,
  otherUserName,
  otherUserId,
  escrowId,
}: ChatBoxWithMediatorProps) {
  const { user } = useAuth();
  const [messages, setMessages] = useState<Message[]>([]);
  const [inputValue, setInputValue] = useState("");
  const [isRecording, setIsRecording] = useState(false);
  const [recordingTime, setRecordingTime] = useState(0);
  const [mediatorRequest, setMediatorRequest] = useState<any>(null);
  ...
  const { data: mediatorRequests } = trpc.mediator.getMediatorRequest.useQuery(
    { requestId: mediatorRequest?.id },
    { refetchInterval: 3000, enabled: !!mediatorRequest }
  );
  ...
  return (
    <div className="flex flex-col h-full gap-4">
      {/* Mediator Panel - Show if mediator request exists */}
      {mediatorRequest && (
        <MediatorPanel
          mediatorRequestId={mediatorRequest.id}
          conversationId={conversationId}
          mediatorId={mediatorRequest.mediatorId}
          status={mediatorRequest.status}
        />
      )}

      {/* Main Chat Box */}
      <div className="flex flex-col h-full bg-white rounded-lg shadow-lg flex-1">
        {/* Header */}
          ...
          <div className="flex items-center justify-between">
            ...
            {mediatorRequest && (
              <div className="flex items-center gap-2 bg-blue-500 px-3 py-1 rounded-full">
                ...
                <span className="text-xs font-semibold">وسيط نشط</span>
              </div>
            )}
          </div>
        </div>
              ...
              >
                <div
                  className={`max-w-xs px-4 py-2 rounded-lg ${
                    ...
                    message.isMediatorMessage
                      ? "border-2 border-amber-400 bg-amber-50 text-gray-800"
                      : ""
                  }`}
                >
                  ...
                  {message.messageType === "audio" && message.mediaUrl && (
                    <div className="flex items-center gap-2">
                      <audio controls className="h-8" src={message.mediaUrl} />
        ...
        <div className="border-t bg-white p-4 rounded-b-lg">
          ...
          <div className="space-y-2">
            ...
            <RequestMediatorButton
              conversationId={conversationId}
              onMediatorRequested={() => {
                // Refetch mediator requests
          ...
          <p className="text-xs text-gray-500 mt-2">
            ...
            للتحقق من النزاعات. استخدم زر "طلب وسيط" إذا احتجت إلى مساعدة.
          </p>
        </div>
      </div>
    </div>
